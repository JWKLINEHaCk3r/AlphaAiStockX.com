...
const fixes = [
  ...
  { pattern: /interface RecentTrade\s*{[^}]*id:\s*string[^}]*}/gs, replacement: (match) => match.replace(/id:\s*string/, 'id: number'), description: 'Fix RecentTrade id type to be compatible with Trade interface'
  };
  ...
  // Add optional properties to interfaces
  {
    ...
    {${newBody}}`; }, description: 'Add optional markers to commonly optional interface properties'
  };
  
  // Replace explicit any types with more specific types where possible
  { pattern: /:\s*any(\s*[,,\]\)\n])/g, replacement: ': unknown$1', description: 'Replace any with unknown for better type safety'
  };
...
const specificFixes = [
  // Fix Trade interface id type consistency { files: ['**/*Trade*.tsx', '**/AutoTradeBot.tsx'], pattern: /id:\s*string/g, replacement: 'id: number', description: 'Ensure Trade interface uses number IDs consistently'
  };
  
  // Fix optional property access { files: ['**/*.tsx'], pattern: /(\w+)\.(\w+)\.toFixed\(/g, replacement: '($1?.$2 || 0).toFixed(', description: 'Add safe navigation for toFixed calls'
  };
  
  // Fix array access safety { files: ['**/*.tsx'], pattern: /(\w+)\[(\d+)\](?!\?)/g, replacement: '$1?.[$2]', description: 'Add safe array access'
  }
];
...
function findFiles(dir, pattern, files = []) {
  ...
  for (const entry of entries) {
    const fullPath = path.join(dir, entry.name); if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
 ...
 function applyFixes(filePath) { let content = fs.readFileSync(filePath, 'utf8');
  let modified = false;
