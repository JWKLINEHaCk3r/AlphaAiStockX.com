...
const PerformanceMonitor = memo(({
  ...
  const systemHealth = useMemo(() => {
    ...
    if (average >= 90) return { status: 'excellent', color: 'text-green-400', bgColor: 'bg-green-500/20' };
    if (average >= 75) return { status: 'good', color: 'text-yellow-400', bgColor: 'bg-yellow-500/20' };
    if (average >= 50) return { status: 'warning', color: 'text-orange-400', bgColor: 'bg-orange-500/20' };
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-gray-800/50 rounded-lg p-3">
              ...
              <div className="space-y-1">
                ...
                <div className={`text-xs ${metrics.responseTime < 100 ? 'text-green-400' : metrics.responseTime < 300 ? 'text-yellow-400' : 'text-red-400'}`}>
                  {metrics.responseTime < 100 ? 'Excellent' : metrics.responseTime < 300 ? 'Good' : 'Slow'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800/30 rounded-lg p-4 border border-gray-700/50">
            ...
            <div className="text-xs text-gray-400">
              System is {systemHealth.status === 'excellent' ? 'performing optimally' :
                        systemHealth.status === 'good' ? 'running smoothly' :
                        systemHealth.status === 'warning' ? 'experiencing minor issues' :
