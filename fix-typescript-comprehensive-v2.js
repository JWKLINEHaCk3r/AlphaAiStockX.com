...
// #!/usr/bin/env node
/** * Comprehensive TypeScript fixes for AlphaAiStockX * This script fixes 'any' types, unused variables, and other TypeScript issues
...
function addImportsIfNeeded(filePath, content) {
  // Check if file needs imports const needsImports = anyTypeReplacements.some(replacement =>  replacement.to && content.includes(replacement.to.split('(')[0])
  );
  
  if (!needsImports) return content;
   // Determine which import set to use let importToAdd = ''; if (filePath.includes('app/components')) { importToAdd = importStatements['app/components']; } else if (filePath.includes('app/services')) { importToAdd = importStatements['app/services'];
  }
  
  if (!importToAdd) return content;
   // Check if imports already exist if (content.includes('from \'../../types/trading-types\'') ||  content.includes('from \'../types/trading-types\'')) {
    return content;
  }
  ...
  if (match) {
    const existingImports = match[1]; const restOfFile = content.slice(match[1].length); return existingImports + importToAdd + '\\n\\n' + restOfFile;
  } return importToAdd + '\\n\\n' + content;
}
...
function fixFile(filePath) {
 ...
 function main() { console.log('🔧 Starting comprehensive TypeScript fixes...');
