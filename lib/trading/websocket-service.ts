...
export class WebSocketService {
  ...
  public notifyPortfolioUpdate(userId: string, portfolio: PortfolioUpdate): void {
    ...
    if (!subscribedUsers || !subscribedUsers.has(userId)) return;
  ...
  private async validateToken(token: string, userId: string): Promise<boolean> {
    try {
      ...
      SecurityAudit.logSecurityEvent({
        ...
        userId,
        details: { action: 'token_validation_failed', error: error instanceof Error ? error.message : 'Unknown error' },
      });
      return false;
    }
  }
  ...
  public getConnectionStats(): {
    ...
    return {
      totalConnections: Array.from(this.connectedClients.values()).reduce((sum, sockets) => sum + sockets.size, 0),
