...
export interface OrderRequest {
  ...
  symbol: string;
  quantity?: number, notional?: number, side: 'buy' | 'sell', type: 'market' | 'limit' | 'stop' | 'stop_limit' | 'trailing_stop', timeInForce: 'day' | 'gtc' | 'opg' | 'cls' | 'ioc' | 'fok',
  limitPrice?: number;
  stopPrice?: number;
  trailPrice?: number;
  trailPercent?: number;
...
export interface PositionSummary {
  ...
  averageEntryPrice: number, currentPrice: number, side: 'LONG' | 'SHORT',
    changeToday: number;
...
export class TradingService {
 ...
 // Update portfolio cash balance for buy orders; if (orderRequest.side === 'buy' && orderRequest.type === 'market') {
    ...
    userId: this.config.userId, details: { action: 'order_placed',
 ...
 SecurityAudit.logSecurityEvent({ type: 'trading_activity',
    userId: this.config.userId, details: { action: 'order_canceled',
          orderId;
        };
      });
    } catch (error) { SecurityAudit.logSecurityEvent({ type: 'trading_error',
  ...
  async getHistoricalData(params: { symbols: string[], timeframe: '1Min' | '5Min' | '15Min' | '30Min' | '1Hour' | '1Day',
    start?: string;
    end?: string;
    limit?: number;
  }) {
  ...
  private validateOrderRequest(orderRequest: OrderRequest): void { if (!orderRequest.symbol || orderRequest.symbol.length === 0) { throw new Error('Symbol is required');
    }
 if (!orderRequest.quantity && !orderRequest.notional) { throw new Error('Either quantity or notional amount is required');
    }
 if (orderRequest.quantity && orderRequest.quantity <= 0) { throw new Error('Quantity must be positive');
    }
 if (orderRequest.notional && orderRequest.notional <= 0) { throw new Error('Notional amount must be positive');
    } if (orderRequest.type === 'limit' && !orderRequest.limitPrice) { throw new Error('Limit price is required for limit orders');
    }
 if ( (orderRequest.type === 'stop' || orderRequest.type === 'stop_limit') &&;
      !orderRequest.stopPrice; ) { throw new Error('Stop price is required for stop orders');
    }
  }
 ...
 if (orderRequest.notional && orderRequest.notional > buyingPower) { throw new Error('Insufficient buying power');
      }

      if (orderRequest.quantity && orderRequest.limitPrice) {
        const orderValue = orderRequest.quantity * orderRequest.limitPrice; if (orderValue > buyingPower) { throw new Error('Insufficient buying power');
        }
      }
    }
 // Check position for sell orders; if (orderRequest.side === 'sell' && orderRequest.quantity) {
      const position = await this.alpaca.getPosition(orderRequest.symbol); if (!position || parseFloat(position.qty) < orderRequest.quantity) { throw new Error('Insufficient position for sell order');
      }
    }
  }
