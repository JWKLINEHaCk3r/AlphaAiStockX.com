...
import { InputValidator, SecurityAudit, RateLimiter } from '@/lib/security';
import { z } from 'zod';
...
const RATE_LIMITS = {
  orders: { maxRequests: 100, windowMs: 60000 }, // 100 orders per minute
  portfolio: { maxRequests: 60, windowMs: 60000 }, // 60 requests per minute
  marketData: { maxRequests: 300, windowMs: 60000 }, // 300 requests per minute
};
...
// Trading orders API
export async function handleTradingOrders(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    try {
      ...
      if (!session?.user?.tradingEnabled) {
        return res.status(403).json({ error: 'Trading not enabled for this account' });
      }
      ...
      SecurityAudit.logSecurityEvent({
        ...
        details: { action: 'order_placed', order },
      });
...
// Portfolio API
export async function handlePortfolio(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
