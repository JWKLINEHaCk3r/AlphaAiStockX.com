    ...
    try {  
      ...
      if (content.includes("'use client'") || content.includes('"use client"')) {
        ...
        if (useClientLineIndex > 0) {
          ...
          const useClientMatch = lines[useClientLineIndex].match(/(["'])use client\1/);
          if (useClientMatch) {
            ...
            if (lines[useClientLineIndex].trim() === '') {
              ...
              } catch (error) { console.error(error); } catch (error) { console.error(error); }// Add "use client" at the very top
