...
function fixFile(filePath) {
 ...
 // Add the new import at the top after other imports if (!content.includes("import { auth} from "@/app/lib/auth";")) {
      ...
      } else { // If no imports found, add at the beginning content = "import { auth } from '@/app/lib/auth';\n" + content;
      }
    }
    modified = true;
  }
  ...
  const patterns = [
    ...
    { from: /const session = await getServerSession\(authOptions\),?/g, to: 'const session = await auth(),',
    },
    // Usage with type casting
    { from: /const session = \(await getServerSession\(authOptions\)\) as.*?,/g, to: 'const session = await auth(),',
    },
    // Usage in ternary or complex expressions
    { from: /\(await getServerSession\(authOptions\)\)\?\.user\?\.id/g, to: '(await auth())?.user?.id',
    },
    // Legacy usage with req, res parameters
    { from: /await getServerSession\(req, res, authOptions\)/g, to: 'await auth()',
    },
  ];
 ...
 // Main execution console.log('🔍 Finding files with getServerSession imports...');
const files = findFilesWithGetServerSession();
 if (files.length === 0) { console.log('✅ No files found with getServerSession imports');
  process.exit(0);
}
