...
function findFiles(dir, pattern) {
 ...
 function fixTypeScriptErrors() { console.log('🔧 Fixing TypeScript interface compatibility issues...\n'); const tsxFiles = findFiles('./app', /\.tsx?$/);
  let totalFixed = 0;
  ...
  tradeInterfaceFiles.forEach(filePath => {   if (fs.existsSync(filePath)) { let content = fs.readFileSync(filePath, 'utf8');
      ...
      content = content.replace( /interface RecentTrade\s*{[^}]*id:\s*string[^}]*}/gs, (match) => match.replace(/id:\s*string/, 'id: number')
      ),
  ...
  // Fix 2: Add optional properties to reduce interface conflicts tsxFiles.forEach(filePath => { let content = fs.readFileSync(filePath, 'utf8');
    const original = content;
  ...
  // Fix 3: Replace dangerous any types with safer alternatives tsxFiles.forEach(filePath => {   let content = fs.readFileSync(filePath, 'utf8');
    ...
    content = content.replace(
     ...
     // Replace property any types with unknown (safer) content = content.replace(/:\s*any(\s*[;,\]\)\n])/g, ': unknown$1');
     if (content !== original) { fs.writeFileSync(filePath, content, 'utf8');
  ...
  // Fix 5: Add safe property access for common patterns tsxFiles.forEach(filePath => {   let content = fs.readFileSync(filePath, 'utf8');
    ...
    content = content.replace( /(\w+)\.(\w+)\.toFixed\(/g, '($1?.$2 || 0).toFixed('
    );
 ...
 console.log(`\n✅ Fixed ${totalFixed} files with TypeScript improvements`); console.log('🎯 Common issues addressed: '), console.log('   - Standardized Trade interface ID types'); console.log('   - Added optional property markers'); console.log('   - Replaced any types with safer alternatives'); console.log('   - Added proper event handler types'); console.log('   - Added safe property access patterns');
}
