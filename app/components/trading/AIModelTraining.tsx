...
import { Brain, Cpu, Database, Zap, BarChart3, Target, Settings, Play, Pause, Square } from "lucide-react"
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setTrainingConfig((prev) => ({ ...prev, modelType: value }))}
                >
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setTrainingConfig((prev) => ({ ...prev, targetPrediction: value }))}
                >
                  <SelectTrigger className="bg-gray-800/30 border-red-500/30 text-gray-200">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setTrainingConfig((prev) => ({ ...prev, epochs: Number.parseInt(e.target.value) }))
                    }
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setTrainingConfig((prev) => ({ ...prev, datasetSize: value }))}
                >
                  <SelectTrigger className="bg-gray-800/30 border-red-500/30 text-gray-200">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-gray-800/30 rounded-lg border border-red-500/20">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Badge className={getStatusColor(trainingStatus)}>{trainingStatus.toUpperCase()}</Badge>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-gray-200">{modelTypes.find((t) => t.id === trainingConfig.modelType)?.name}</p>
                    </div>
                    <div>
              ...
              <div className="flex space-x-2">
                ...
                {trainingStatus === "training" && (
                  <Button onClick={pauseTraining} className="flex-1 bg-yellow-600 hover:bg-yellow-700">
                ...
                {trainingStatus === "completed" && (
                  <Button onClick={() => setTrainingStatus("idle")} className="flex-1 bg-green-600 hover:bg-green-700">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="text-center p-3 bg-gray-800/50 rounded-lg border border-red-500/20">
                  ...
                  <p className="text-lg font-bold text-purple-400">
                    {(models.reduce((sum, m) => sum + m.profitability, 0) / models.length).toFixed(1)}%
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex items-center space-x-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        Last trained: {model.lastTrained} • {model.dataPoints} data points • {model.trainingTime}
                      </p>
                    </div>

                    <div className="text-center">
                      <p className="text-gray-400 text-sm">Accuracy</p>
                      <p className="text-green-400 font-bold text-lg">{model.accuracy.toFixed(1)}%</p>
                    </div>

                    <div className="text-center">
                      <p className="text-gray-400 text-sm">Win Rate</p>
                      <p className="text-blue-400 font-bold text-lg">{model.performance.winRate.toFixed(1)}%</p>
                    </div>

                    <div className="text-center">
                      <p className="text-gray-400 text-sm">Sharpe Ratio</p>
                      <p className="text-purple-400 font-bold text-lg">{model.performance.sharpeRatio.toFixed(2)}</p>
                    </div>

                    <div className="text-center">
                      <p className="text-gray-400 text-sm">Profitability</p>
                      <p className="text-red-400 font-bold text-lg">{model.profitability.toFixed(1)}%</p>
                    </div>
                  </div>

                  <div className="flex items-center space-x-2">
                    {model.status === "completed" && (
                      <Button onClick={() => deployModel(model.id)} className="bg-green-600 hover:bg-green-700">
                        Deploy Model
                      </Button>
                    )}
                ...
                <div className="mt-4 pt-4 border-t border-gray-700/30">
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-gray-200 font-semibold">{model.performance.precision.toFixed(3)}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Recall:</span>
                      <p className="text-gray-200 font-semibold">{model.performance.recall.toFixed(3)}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">F1 Score:</span>
                      <p className="text-gray-200 font-semibold">{model.performance.f1Score.toFixed(3)}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Max Drawdown:</span>
                      <p className="text-red-400 font-semibold">{model.performance.maxDrawdown.toFixed(1)}%</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Predictions:</span>
                      <p className="text-gray-200 font-semibold">{model.predictions.toLocaleString()}</p>
                    </div>
                    <div>
