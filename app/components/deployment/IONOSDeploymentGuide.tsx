    ...
    <div>
      ...
      <ol>
        ...
        <li>
          ...
          <p>
            Configure your IONOS server to serve the built application files (usually in the <code>/dist</code> or <code>/build</code> directory).
            You may need to configure a reverse proxy (e.g., using Nginx or Apache) to route requests to your application.
          </p>
        </li>
        <li>
          <strong>Start your application:</strong>
          <p>
            Start your application using a process manager like PM2 or systemd to ensure it stays running.
            For example, if you have a <code>start</code> script in your <code>package.json</code>:
          </p>
        ...
        <li>
          ...
          <p>
            Create an Nginx configuration file (e.g., <code>/etc/nginx/sites-available/your-app</code>) with the following content:
          </p>
          <pre><code>
server {\
    listen 80;
...
sudo systemctl reload nginx
      ...
      <p>
        If you encounter any issues, check your server logs, Nginx configuration, and application logs for errors.
      </p>
    </div>
  )
}
