...
export default function AIGlobalHeatmap() {
  ...
  const fetchHeatmap = () => {
    ...
    setTimeout(() => {
      setHeatmap([
        ...
        { region: "Emerging Mkts", trend: "+0.9%", aiNote: "AI: Capital inflows, bullish momentum." },
      ])
      setLoading(false)
    }, 1200)
  }
  ...
  return (
    <div className="futuristic-card holo-shimmer p-6 mb-8">
      ...
      <p className="text-slate-300 mb-4">Real-time, AI-annotated, interactive heatmap of global markets.</p>
      <button className="holo-btn px-6 py-2 font-bold mb-4" onClick={fetchHeatmap} disabled={loading}>
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
          {heatmap.map((h, i) => (
            <div key={i} className="futuristic-card p-4 animated-neon-border">
              ...
              <div className={`font-bold ${h.trend.startsWith("-") ? "text-red-400" : "text-emerald-300"}`}>Trend: {h.trend}</div>
