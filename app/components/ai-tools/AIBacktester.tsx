...
export default function AIBacktester() { const [loading, setLoading] = useState(false);
  ...
  const runBacktest = () => {
    setLoading(true);
    setTimeout(() => { setResults({ strategy: 'AI Momentum + News', period: '2022-2025', trades: 124, winRate: '72.6%', totalReturn: '+184%', aiNote: 'AI: Outperformed S&P 500 by 4.2x. Most gains from tech and AI sectors.'
      });
      setLoading(false);
    }, 1200);
  };
  ...
  return (
    <div className="futuristic-card holo-shimmer p-6 mb-8">
      ...
      <p className="text-slate-300 mb-4">
        Lightning-fast
               multi-strategy backtesting with explainable AI results
      </p>
      <button
        className="holo-btn px-6 py-2 font-bold mb-4" onClick={runBacktest}
        disabled={loading} > {loading ? 'Backtesting...' : 'Run Backtest'}
      </button>
      {results && (
