    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-green-900/20 to-emerald-900/20 border-2 border-green-400/50 backdrop-blur-xl shadow-2xl shadow-green-500/25">
        <CardHeader>
          <CardTitle className="text-white flex items-center text-2xl">
            <DollarSign className="h-8 w-8 mr-3 text-green-400 animate-pulse" />💰 HIGH-YIELD MONEY MARKET ACCOUNT
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="text-center p-6 bg-gradient-to-r from-green-400/20 to-emerald-500/20 rounded-xl border-2 border-green-400/30 shadow-lg shadow-green-500/25">
              ...
              <p className="text-3xl font-bold text-green-400">${moneyMarketBalance.toLocaleString()}</p>
            ...
            <div className="text-center p-6 bg-gradient-to-r from-purple-400/20 to-pink-500/20 rounded-xl border-2 border-purple-400/30 shadow-lg shadow-purple-500/25">
              ...
              <p className="text-3xl font-bold text-purple-400">${totalInterestEarned.toFixed(2)}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-2 border-green-400/30 backdrop-blur-xl">
          <CardHeader>
            <CardTitle className="text-white flex items-center">
              <ArrowUpRight className="h-6 w-6 mr-2 text-green-400" />💸 Deposit Funds
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button
              ...
              disabled={
                !depositAmount || Number.parseFloat(depositAmount) <= 0 || Number.parseFloat(depositAmount) > balance
              }
        ...
        <Card className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-2 border-blue-400/30 backdrop-blur-xl">
          <CardHeader>
            <CardTitle className="text-white flex items-center">
              <ArrowDownLeft className="h-6 w-6 mr-2 text-blue-400" />💳 Withdraw Funds
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-1">Available: ${moneyMarketBalance.toLocaleString()}</p>
            </div>
            ...
            <div className="p-3 bg-blue-500/10 rounded-lg border border-blue-500/30">
              ...
              <p className="text-xs text-gray-300">No penalties • Same-day transfers • Keep earning until withdrawal</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-2 border-purple-400/30 backdrop-blur-xl">
        <CardHeader>
          <CardTitle className="text-white flex items-center">
            <Target className="h-6 w-6 mr-2 text-purple-400" />📈 Earnings Projections
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center p-4 bg-purple-500/10 rounded-lg border border-purple-500/30">
              ...
              <p className="text-2xl font-bold text-white">${projectedMonthlyEarnings.toFixed(2)}</p>
            ...
            <div className="text-center p-4 bg-cyan-500/10 rounded-lg border border-cyan-500/30">
              ...
              <p className="text-2xl font-bold text-white">${(moneyMarketBalance * Math.pow(1.015, 5)).toFixed(2)}</p>
      ...
      <Card className="bg-gradient-to-r from-gray-900/90 to-black/90 border-2 border-cyan-400/30 backdrop-blur-xl">
        <CardHeader>
          <CardTitle className="text-white flex items-center">
            <Activity className="h-6 w-6 mr-2 text-cyan-400" />📊 Recent Activity
          </CardTitle>
        </CardHeader>
        <CardContent>
          {interestHistory.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400">No transactions yet. Start earning interest by depositing funds!</p>
            </div>
          ) : (
              ...
              {interestHistory.map((transaction) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    >
                      {transaction.type === "interest" && <TrendingUp className="h-4 w-4 text-green-400" />}
                      {transaction.type === "deposit" && <ArrowUpRight className="h-4 w-4 text-blue-400" />}
                      {transaction.type === "withdrawal" && <ArrowDownLeft className="h-4 w-4 text-red-400" />}
                    </div>
                    <div>
