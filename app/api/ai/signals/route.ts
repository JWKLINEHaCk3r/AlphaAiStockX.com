...
export async function GET(request: NextRequest) {
  try {
    ...
    const signals = [];
    const aiModels = [
      'Neural Ensemble v4.1',
      'LSTM Deep Learning',
      'Transformer GPT-Finance',
    ...
    for (let i = 0; i < limit; i++) {
      ...
      signals.push({
        ...
        reasoning: [
          `${model} prediction with ${Math.round(confidence * 100)}% confidence`,
          `RSI at ${Math.round(rsi)}, ${rsi < 30 ? 'oversold' : rsi > 70 ? 'overbought' : 'neutral'} conditions`
        ],
