...
const marketDataQuerySchema = z.object({
  ...
  limit: z.string().transform((val) => Math.min(parseInt(val) || 100, 1000)).default('100'),
  includeSnapshot: z.string().transform((val) => val === 'true').default('true'),
  includeBars: z.string().transform((val) => val === 'true').default('false'),
});
        ...
        queryParams.symbols.map(async (symbol) => {
          try {
            const responses = await Promise.allSettled([
              ...
              queryParams.includeBars ? alpacaClient.getBars({
                symbols: [symbol],
                timeframe: queryParams.interval === '1m' ? '1Min' :
                          queryParams.interval === '5m' ? '5Min' :
                          queryParams.interval === '15m' ? '15Min' :
                          queryParams.interval === '30m' ? '30Min' :
                          queryParams.interval === '1h' ? '1Hour' : '1Day',
...
function calculateTechnicalIndicators(bars: any[]): any {
  ...
  const sma50 = closes.length >= 50 ? closes.slice(-50).reduce((sum, price) => sum + price, 0) / 50 : sma20;
