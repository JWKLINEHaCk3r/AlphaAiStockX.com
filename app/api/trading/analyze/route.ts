...
export async function GET(request: NextRequest) {
  try { return NextResponse.json({ message: 'analyze endpoint is healthy', timestamp: new Date().toISOString(), status: 'active'
    }); } catch (error) { console.error('analyze GET error:', error); return NextResponse.json( { error: 'Internal server error' },
      { status: 500 }
    );
}
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    }) } catch (error) { console.error('analyze POST error:', error); return NextResponse.json( { error: 'Internal server error' },
      { status: 500 }
    );
}
}
