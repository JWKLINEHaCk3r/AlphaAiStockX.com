...
export class DataAggregationService {
  ...
  private async loadMarketKnowledge() {
    const knowledge = {
      marketCycles: {
        bull: {
          duration: '2-7 years',
          characteristics: ['Rising prices', 'High volume', 'Optimism'],
        },
        bear: {
          duration: '6 months - 2 years',
          characteristics: ['Falling prices', 'Fear', 'Pessimism'],
        },
  ...
  private async getNewsAnalysis(symbol: string) {
    ...
    return {
      ...
      impactScore:
        news.reduce((sum, item) => sum + (item.impact === 'high' ? 1 : 0.5), 0) / news.length ||
        0.5,
    };
  }
  ...
  private async getEconomicImpact(symbol: string) {
    ...
    return {
      ...
      overallImpact:
        economicData.reduce((sum, data) => sum + Math.abs(data.change), 0) / economicData.length ||
        0,
    };
  }
  ...
  private calculateOverallMarketSentiment() {
    ...
    const avgSentiment =
      sentimentData.reduce((sum, data) => sum + data.overall, 0) / sentimentData.length || 50;
