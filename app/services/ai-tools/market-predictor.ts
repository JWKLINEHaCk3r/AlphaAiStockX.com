...
export class AIMarketPredictor {
  ...
  private async recognizePatterns(description: string): Promise<TechnicalPattern[]> {
    ...
    bullishPatterns.forEach(pattern => {
      ...
      if (lowerDesc.includes(patternName.replace(/\s+/g, '')) || 
          lowerDesc.includes(patternName)) {
        patterns.push({
          ...pattern,
  ...
  private inferPatternsFromDescription(description: string): TechnicalPattern[] {
    ...
    if (description.includes('bullish') || description.includes('upward') || description.includes('rising')) {
      patterns.push({
        name: 'Bullish Trend',
        type: 'BULLISH',
        confidence: 0.6,
    ...
    if (description.includes('bearish') || description.includes('downward') || description.includes('falling')) {
      patterns.push({
        name: 'Bearish Trend',
        type: 'BEARISH',
        confidence: 0.6,
    ...
    if (description.includes('sideways') || description.includes('consolidation') || description.includes('range')) {
      patterns.push({
        name: 'Consolidation',
        type: 'NEUTRAL',
        confidence: 0.7,
  ...
  private calculateTargetPrice(levels: PriceLevel[], direction: string): number | undefined {
    ...
    const resistanceLevels = levels.filter(l => l.type === 'RESISTANCE').sort((a, b) => a.level - b.level);
    const supportLevels = levels.filter(l => l.type === 'SUPPORT').sort((a, b) => b.level - a.level);
  ...
  private async generateCombinedAnalysis(
    ...
    - Sector Performance: ${Object.entries(marketContext.sectorRotation).map(([s, p]) => `${s}: ${p.toFixed(1)}%`).join(', ')}
    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert market analyst combining technical, fundamental, and macro analysis for comprehensive market predictions.'
          },
          {
            role: 'user',
            content: prompt
  ...
  private extractOpportunities(content: string): string[] {
    ...
    return opportunities.length > 0 ? opportunities : ['Pattern completion potential', 'Market timing opportunity'];
  }
  ...
  async getHistoricalAccuracy(symbol: string): Promise<any> {
    ...
    return {
      ...
      averageConfidence: (history.reduce((sum, h) => sum + h.confidence, 0) / history.length * 100).toFixed(1) + '%'
    };
  }
