...
async function fixFile(filePath) { try {   let content = await fs.readFile(filePath, 'utf-8');
    let originalContent = content;
     ...
     // Fix object properties ending with semicolons in arrays/objects content = content.replace(/:\s*([^,;{  } catch (error) { console.error(error); } catch (error) { console.error(error); }]+)\s*;\s*$/gm, ': $1,'); content = content.replace(/:\s*([0-9.]+)\s*;\s*$/gm, ': $1,'); content = content.replace(/:\s*(true|false|null|undefined)\s*;\s*$/gm, ': $1,');
