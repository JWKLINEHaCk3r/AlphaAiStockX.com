import React from 'react'; // Cypress support commands for E2E testing; import '@testing-library/cypress/add-commands';
...
//   }
// }
 // Login command; Cypress.Commands.add('login', (email, password) => { cy.visit('/login'); cy.get('[data-testid="email-input"]').type(email); cy.get('[data-testid="password-input"]').type(password); cy.get('[data-testid="login-button"]').click(); cy.url().should('not.include', '/login');
});
 // Logout command; Cypress.Commands.add('logout', () => { cy.get('[data-testid="user-menu"]').click(); cy.get('[data-testid="logout-button"]').click(); cy.url().should('include', '/');
});
 // Navigate to trading; Cypress.Commands.add('navigateToTrading', () => { cy.get('[data-testid="nav-trading"]').click(); cy.url().should('include', '/trading');
});
 // Place order; Cypress.Commands.add('placeOrder', orderData => { cy.get('[data-testid="symbol-input"]').type(orderData.symbol); cy.get('[data-testid="quantity-input"]').type(orderData.quantity.toString()); cy.get('[data-testid="order-type-select"]').select(orderData.type); cy.get('[data-testid="place-order-button"]').click();
});
